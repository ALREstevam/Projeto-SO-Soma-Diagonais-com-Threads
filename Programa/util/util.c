...
void OutputSumToUser(MatrixDescriber *matrix, ArrayDescriber *rspArr){
	...
	for(i = 0; i < matrix->diagNum; i++){
        ...
        printf("\t%c  (%4d,%4d)  %c     %5d     %c %10.5g  \t   %c    %11lu   %c    %9d     %c",vertSepChar,c.mpos, c.npos,vertSepChar, i,vertSepChar, rspArr->data[i].rspi.rsp,vertSepChar, (unsigned long) /*rspArr->data[i].rspi.tid*/ 0, vertSepChar, rspArr->data[i].rspi.countDiagElem, vertSepChar);
